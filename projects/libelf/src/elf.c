...
{
    return elf_newFile_maybe_unsafe(file, size, res);
}

int elf_newFile_maybe_unsafe(const void *file, size_t size, elf_t *res)
{
    res->elfFile = file;
    res->elfSize = size;

    int status = elf_checkFile(res);
    if (status < 0) {
        return status;
    }
    ZF_LOGD("'ELF' file has valid ELF header\n");
    status = elf_checkProgramHeaderTable(res);
    if (status < 0) {
        return status;
    }
    ZF_LOGD("'ELF' file has valid program header table\n");
    status = elf_checkSectionTable(res);
    if (status < 0) {
        return status;
    }
    ZF_LOGD("'ELF' file has valid section header table\n");
    return status;
}
...
const void *elf_getSection(const elf_t *elf, size_t i)
{
    ...
    if (elf->read) {
        return elf_read(elf->pid, section_offset, section_size);
    } else {
        return elf->elfFile + section_offset;
    }
}

void free_section_name(const elf_t *elfFile, const char *str, size_t i) {
    size_t offset = elf_getSectionNameOffset(elfFile, i);
    free(str - offset);
}
...
const void *elf_getSectionNamed(const elf_t *elfFile, const char *str, size_t *id)
{
    ...
    for (size_t i = 0; i < numSections; i++) {
        char *temp_name = elf_getSectionName(elfFile, i);
        if (strcmp(str, temp_name) == 0) {
            if (id != NULL) {
                *id = i;
            }
            if (elfFile->read) free_section_name(elfFile, temp_name, i);
