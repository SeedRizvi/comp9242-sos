...
/**
 * Resumes hardware timer for either amount of time of next timeout after timeout consumption, or 
 * maximum hardware timer value.
 */
void start_time();
...
/* TODO: Currently here for testing. */
typedef struct tdata {
    uint64_t delay;
    uint64_t value;
    unsigned char *timer_vaddr;
} tdata;

void test_timer_func(uint32_t id, void* data);
void test_timer_post_restart(uint32_t id, void* data);
void test_reset_timer(uint32_t id, void* data);

/* Testing functions. */
void test_timeouts(); // Called by test_clock()
void test_clock();