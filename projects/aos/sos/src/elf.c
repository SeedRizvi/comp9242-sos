 ...
 *
 */
static int load_segment_into_vspace(pt_meta_t *pt, seL4_CPtr loadee, const char *src, size_t source_offset, size_t segment_size,
                                    size_t file_size, uintptr_t dst, seL4_CapRights_t permissions, bool use_file, int pid)
{
    ...
    while (pos < segment_size) {
        ...
        if (loadee_frame == seL4_CapNull) {
            ZF_LOGE("Error: ELF: Failed to alloc slot\n");
            return -1;
        }

        /* allocate the untyped for the loadees address space */
        ft_lock();
        frame_ref_t frame = alloc_frame_pt(pt, dst);
        if (frame == NULL_FRAME) {
            ft_unlock();
            ZF_LOGE("Error: ELF: Failed to alloc frame\n");
            return -1;
        }
        ...
        if (err != seL4_NoError) {
            ft_unlock();
            ZF_LOGE("Error: ELF: Failed to untyped reypt\n");
            return -1;
        }

        /* map the frame into the loadee address space */
        err = sos_map_frame(pt, loadee_frame, loadee, loadee_vaddr, permissions, seL4_ARM_Default_VMAttributes, frame);
        ...
        if (already_mapped) {
            ...
            ft_unlock();
            ZF_LOGE("Error: ELF: Failed to map into loadee at %p, error %u\n", (void *) loadee_vaddr, err);
            return -1;
        }
        ft_unlock();
        ...
        if (pos < file_size) {
            ...
            if (use_file) {
                char *temp_data = (char *)elf_read(pid, source_offset, file_bytes);
                memcpy(loader_data, temp_data, file_bytes);
                free(temp_data);
            } else {
        ...
        dst += segment_bytes;
        src += segment_bytes;
        source_offset += segment_bytes;
    }

    return 0;
}

int elf_load(pt_meta_t *pt, seL4_CPtr loadee_vspace, elf_t *elf_file)
{
    ...
    for (int i = 0; i < num_headers; i++) {
        ...
        const char *source_addr = elf_file->elfFile + source_offset;
        ...
        ZF_LOGV(" * Loading segment %p-->%p\n", (void *) vaddr, (void *)(vaddr + segment_size));
        int err = load_segment_into_vspace(pt, loadee_vspace, source_addr, source_offset, segment_size, file_size, vaddr,
                                           get_sel4_rights_from_elf(flags), elf_file->read, elf_file->pid);
        if (err) {
