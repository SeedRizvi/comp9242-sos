...
static bool alloc_stack(sos_thread_t *new_thread, seL4_Word *sp)
{
    ...
    for (int i = 0; i < SOS_STACK_PAGES; i++) {
        ...
        (new_thread->stack_arr)[i] = frame_cap;
        (new_thread->stack_arr_ut)[i] = frame;
        ...
        if (err != seL4_NoError) {
            ...
            cspace_delete(&cspace, (new_thread->stack_arr)[i]);
            cspace_free_slot(&cspace, (new_thread->stack_arr)[i]);
            ut_free((new_thread->stack_arr_ut)[i]);
            (new_thread->stack_arr)[i] = seL4_CapNull;
            (new_thread->stack_arr_ut)[i] = NULL;
            return false;
        }
        curr_stack += PAGE_SIZE_4K;
    }
