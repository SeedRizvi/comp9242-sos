...
size_t ut_pages_for_region(ut_region_t region)
{
    ...
    size_t ret = BYTES_TO_4K_PAGES((region.end - region.start) / PAGE_SIZE_4K * sizeof(ut_t));
...
{
    cspace_ut_lock(); // STDSYNCTRACKING
    size_t ret = table.n_4k_untyped * PAGE_SIZE_4K;
    cspace_ut_unlock(); // STDSYNCTRACKING
    return ret;
}
...
ut_t *ut_alloc_4k_untyped(uintptr_t *paddr)
{
    ...
    ZF_LOGV("Allocated %lx, cap %lx", ut_to_paddr(n), n->cap);
...
ut_t *ut_alloc_nb(size_t size_bits, cspace_t *cspace)
{
...
ut_t *ut_alloc(size_t size_bits, cspace_t *cspace) {
    cspace_ut_lock(); // STDSYNCTRACKING
    ut_t *ret = ut_alloc_nb(size_bits, cspace);
    cspace_ut_unlock(); // STDSYNCTRACKING
    return ret;
}
