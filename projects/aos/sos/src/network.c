...
static seL4_CPtr fs_init_ntfn = 0;
static ut_t *fs_init_ntfn_ut = NULL;
...
void dhcp_callback(void *cli, int code)
{
    if (code != PICO_DHCP_SUCCESS) {
        ...
        ZF_LOGE("DHCP negociation failed with code %d\n", code);
        return;
    }
    pico_lock();
...
void network_init(cspace_t *cspace, void *timer_vaddr, seL4_CPtr irq_ntfn)
{
    ...
    error = pico_device_init(&pico_dev, "sos picotcp\n", mac_addr);
    ...
    if (new_notification(&fs_init_ntfn, &fs_init_ntfn_ut, false) != 0) {
        ZF_LOGE("could not initialise file system wait notification\n");
    }

    fs_global_init(); // Initialises components of file system that do not need to be mounted.
...
{
    if (status < 0) {
        ZF_LOGE("mount/mnt call failed with \"%s\"\n", (char *)data);
    }
    ZF_LOGI("Mounted nfs dir %s\n", nfs_dir_buf);
    fs_init(nfs);
    ZF_LOGI("Initialising page file\n");
    spawn(&fs_init_pagefile, NULL, SOS_THREAD_BADGE, false);
    
    fs_init_flag = 1;
    seL4_Signal(fs_init_ntfn);
}

void wait_on_fs_mount() {
    if (fs_init_flag == 0) {
        ZF_LOGV("FS Init Wait Start\n");
        seL4_Wait(fs_init_ntfn, NULL);
        seL4_Signal(fs_init_ntfn);
        ZF_LOGV("FS Init Wait End\n");
    } else {
        ZF_LOGV("FS Already Initialised\n");
    }
}