                            ...
                            seL4_CPtr free_slots[MAPPING_SLOTS], seL4_Word *used)
{
    ...
    seL4_Error ret = map_frame_impl(cspace, frame_cap, vspace, vaddr, rights, attr, free_slots, used);
    return ret;
}
...
seL4_Error map_frame(cspace_t *cspace, seL4_CPtr frame_cap, seL4_CPtr vspace, seL4_Word vaddr,
                     seL4_CapRights_t rights, seL4_ARM_VMAttributes attr)
{
    seL4_Error ret = map_frame_impl(cspace, frame_cap, vspace, vaddr, rights, attr, NULL, NULL);
    return ret;
}


seL4_Error sos_map_frame(pt_meta_t *pt, seL4_CPtr frame_cap, seL4_CPtr vspace, seL4_Word vaddr, seL4_CapRights_t rights, seL4_ARM_VMAttributes attr, frame_ref_t frame_ref) {
    // align vaddr
    vaddr = ROUND_DOWN(vaddr, PAGE_SIZE_4K);
    ...
    for (size_t i = 0; i < MAPPING_SLOTS && err == seL4_FailedLookup; i++) {
        ...
        /* Allocate missing hardware paging structure and track in process PT */
        seL4_CPtr slot;
        slot = cspace_alloc_slot(&cspace);
        if (slot == seL4_CapNull) {
            ZF_LOGE("No cptr to alloc paging structure\n");
        ...
        case SEL4_MAPPING_LOOKUP_NO_PT:
            err = retype_map_pt(&cspace, vspace, vaddr, ut->cap, slot);
            if (pt_track_intermediate(pt, vaddr, slot, ut, 3)) {
                ZF_LOGE("Error: can not track allocation PT\n");
                return 1;
            }
            break;
        case SEL4_MAPPING_LOOKUP_NO_PD:
            err = retype_map_pd(&cspace, vspace, vaddr, ut->cap, slot);
            if (pt_track_intermediate(pt, vaddr, slot, ut, 2)) {
                ZF_LOGE("Error: can not track allocation PD\n");
                return 1;
            }
            break;
        case SEL4_MAPPING_LOOKUP_NO_PUD:
            err = retype_map_pud(&cspace, vspace, vaddr, ut->cap, slot);
            if (pt_track_intermediate(pt, vaddr, slot, ut, 1)) {
                ZF_LOGE("Error: can not track allocation PUD\n");
                return 1;
            }
            break;
        }
    ...
    // Add corresponding entry to our page table if mapping was successful.
    if (!err) {
        add_pt_entry(pt, vaddr, frame_ref, frame_cap);
    }
    return err;
}
...
void *sos_map_device(cspace_t *cspace, uintptr_t addr, size_t size)
{
    ...
    for (uintptr_t curr = addr; curr < (addr + size); curr += PAGE_SIZE_4K) {
        ...
        if (ut == NULL) {
            ZF_LOGE("Failed to find ut for phys address %p\n", (void *) curr);
            return NULL;
        }
        ...
        if (err != seL4_NoError) {
            ZF_LOGE("Failed to map device frame at %p\n", (void *) device_virt);
