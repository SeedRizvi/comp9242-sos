...
#define NUMPAGES	512ul
#define ITERATIONS 	5

int sparse[NUMPAGES][PAGESIZE/sizeof(int)];

int thrash(int argc, char *argv[]) {
	int i,j;

	printf("Entering the thrash program configured with %ld byte array\n", 
		NUMPAGES * PAGESIZE);

	/* use only the first element in the row */
	...
	for (i=NUMPAGES-1; i>=0; i--) {
		if (sparse[i][0]!=i+ITERATIONS) {
			printf("BAD NEWS!!! - your vm system has a bug!\n");
			exit(1);
		}
	}

	printf("Check of array content passed!\n");
