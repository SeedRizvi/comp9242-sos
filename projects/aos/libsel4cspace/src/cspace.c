...
int cspace_create_two_level(cspace_t *bootstrap, cspace_t *target, cspace_alloc_t alloc)
{
    ...
    int ret = cspace_create(bootstrap, target, true, alloc);
...
int cspace_create_one_level(cspace_t *bootstrap, cspace_t *target)
{
    ...
    cspace_ut_lock(); // STDSYNCTRACKING
    int ret = cspace_create(bootstrap, target, false, empty);
    cspace_ut_unlock(); // STDSYNCTRACKING
    return ret;
}

void cspace_destroy_nb(cspace_t *cspace)
{
...
void cspace_destroy(cspace_t *cspace) {
    cspace_ut_lock(); // STDSYNCTRACKING
    cspace_destroy_nb(cspace);
    cspace_ut_unlock(); // STDSYNCTRACKING
}

seL4_CPtr cspace_alloc_slot_nb(cspace_t *cspace)
{
    assert(cspace != NULL);
...
seL4_CPtr cspace_alloc_slot(cspace_t *cspace) {
    cspace_ut_lock(); // STDSYNCTRACKING
    seL4_CPtr ret = cspace_alloc_slot_nb(cspace);
    cspace_ut_unlock(); // STDSYNCTRACKING
    return ret;
}

void cspace_free_slot_nb(cspace_t *cspace, seL4_CPtr cptr)
{
...
void cspace_free_slot(cspace_t *cspace, seL4_CPtr cptr) {
    cspace_ut_lock(); // STDSYNCTRACKING
    cspace_free_slot_nb(cspace, cptr);
    cspace_ut_unlock(); // STDSYNCTRACKING
}

seL4_Error cspace_untyped_retype_nb(cspace_t *cspace, seL4_CPtr ut, seL4_CPtr target,
...
seL4_Error cspace_untyped_retype(cspace_t *cspace, seL4_CPtr ut, seL4_CPtr target, seL4_Word type, size_t size_bits) {
    cspace_ut_lock(); // STDSYNCTRACKING
    seL4_Error ret = cspace_untyped_retype_nb(cspace, ut, target, type, size_bits);
    cspace_ut_unlock(); // STDSYNCTRACKING
    return ret;
}